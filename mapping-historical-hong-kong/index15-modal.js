...
function updateSlider() {
  ...
  document.getElementById("year-modal").textContent = year;
...
window.addEventListener("load", (event) => {
  ...
  p.getHeader().then((h) => {
    ...
    // Make sure the map resizes properly with the window
      ...
      p.getMetadata().then((md) => {
        ...
        layers.forEach((item) => {
          ...
          if (
            ...
            let textField = ["coalesce", ["get", "name"], ["get", "Name"], " "];
            let textSize = [
              "interpolate",
              ["linear"],
              ["zoom"],
    ...
    function addHover(lotsLayerName) {
      ...
      map.on("click", hoverLayerName, (e) => {
        ...
        var infoModal = document.getElementById("information-modal");
        ...
        if (window.innerWidth <= 1200) {
          const dataPanelModal = new bootstrap.Modal(
            document.getElementById("dataPanelModal")
          );
          dataPanelModal.show();
        }
      });
